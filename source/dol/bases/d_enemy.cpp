...
const float l_other_speedX[] = { 1.5f, -1.5f };
...
dEn_c::dEn_c() : m24(0), mFootRelated(mVec3_c(0.0f, 0.0f, 0.0f)), m46(false), m48(false), m4a(false),
mBlockHit(), mBoyon(this), mIceMng(this), mCounterCont(1), mFumiCheck(this) {
    mAccelY = -0.1875f;
    ...
    mCounterCont.mCounters3[0] = 0;
    mCounterCont.mCounters3[1] = 0;
}
...
int dEn_c::preExecute() {
    ...
    if (mBlockHit.mData.mIsInit) {
        ...
        for (int i = 0; i < 2; i++) {
            if (mCounterCont.mCounters2[i] != 0) {
                mCounterCont.mCounters2[i]--;
            }
        }
        for (int i = 0; i < 2; i++) {
            if (mCounterCont.mCounters3[i] != 0) {
                mCounterCont.mCounters3[i]--;
            }
        }
    }
    calcBoyonScale();
...
void dEn_c::posMove() {
    ...
    if (m48) {
        ...
        float cos = nw4r::math::CosS(ang);
        float sin = nw4r::math::SinS(ang);
        move.set(speed * cos, mSpeed.y + speed * sin, mSpeed.z);
        if (m49) {
...
void dEn_c::normal_collcheck(dCc_c *cc1, dCc_c *cc2) {
    ...
    if (kind == 3) {
        ...
        if (o1->me8 & 0x1000000 || !CeilCheck(o1->mPos.y, cc1)) {
...
bool dEn_c::CeilCheck(float y, dCc_c *cc) {
    float t = y + cc->mCcData.mOffsetY - cc->mCcData.mHeight;
    float bgY = dBgParameter_c::getInstance()->mPos.y;
    return t > bgY;
}
...
int dEn_c::Enfumi_check(dCc_c *cc1, dCc_c *cc2, int step) {
    ...
    int fumiRes = mFumiCheck.operate(fumiInfo, 1);
...
bool dEn_c::checkComboClap(int max) {
    static const int claps[] = { 7, 7, 4, 0 };
    return claps[mCounterCont.mfc] <= max;
}
...
void dEn_c::block_hit_init() {
    ...
    dAudio::g_pSndObjMap->startSound(SE_EMY_DOWN, mPos, 0);

    mBlockHit.set((block_hit_data_s) {
        l_other_speedX[(u8) (int) tmp],
        3.0f,
        -4.0f,
        -0.1875f,
        &StateID_DieFall,
